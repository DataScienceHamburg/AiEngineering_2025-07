...
#%% restore model
if not DO_TRAINING:
    state_dict = torch.load("binary_classification.pt", map_location=torch.device(DEVICE))
    print(model.load_state_dict(state_dict=state_dict))

# %% 
import seaborn as sns
if DO_TRAINING:
    sns.lineplot(data=[train_losses, val_losses])
...
with torch.no_grad():
    for (X_test_batch, y_test_batch) in test_dataloader:
        ...
        y_test_pred_class.append(pred_classes.cpu())
        y_test_true.append(y_test_batch)

# Concatenate all batches
y_test_pred_class = torch.cat(y_test_pred_class).squeeze()
...
classifier.fit(y_test_true, y_test_pred_class)
classifier.score(y_test_true, y_test_pred_class)